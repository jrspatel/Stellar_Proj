...
def model_registry(params, X, y):
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.sklearn.log_model(
                sk_model=model,
                artifact_path="dt-model",  # Specify your desired artifact path
                registered_model_name="best_dt",
...
def dt_hyperoptimization():
    ...
    params_dt = {'max_depth': hp.randint('max_depth',100,201),
                'min_samples_split': hp.randint('min_samples_split',20, 31)} 


    print("====================== Executing {} ==================".format('DECISION TREE MODEL'))
    space = params_dt
        
    partial_function = partial(objective, X= X_train, y=y_train)

    with open(os.path.join("Stellar_Proj/mlflow", "model_param_dt.json"), 'r') as json_file:
        data = json.load(json_file)
    best_params = (data['decision_tree'])
    
    model_registry(best_params, X_train, y_train)
    #model_version = get_model_version("best_dt", stage="Staging")

    client = MlflowClient()
    client.update_registered_model(
        name="best_dt",
        description="This Decision TREE model data is used to classify star, galaxy. The data consists of 18 features"
    )
